<template>
  <ProTable :request="handleRequest" :query-metadata="queryMetadata" ref="proTableInstance">
    <template #actions>
      <ProActionButton
        :pd="PD.info.popularMedicine.added"
        type="primary"
        plain
        icon="Plus"
    ...
    <ProTableActionsColumn width="120">
      <template #default="scope">
        <ProTableActionButton
          :pd="PD.info.popularMedicine.edit"
          link
          type="primary"
        ...
        <ProTableActionButton
          :pd="PD.info.popularMedicine.delete"
          link
          type="danger"
...
<script setup lang="ts">
  ...
  const [queryMetadata, proTableInstance, handleRequest] = useProTable(
    ...
    async ([currentPage, pageSize], query) => {
      const { dataList, total } = await reqPopularMedicineList({ currentPage, pageSize, ...query })
  ...
  // 编辑 or 新增
  const [proDialogFormInstance, handleAddedOrUpdate] = useProDialogForm<PopularMedicine>(
    () => ({
      title: {
        label: '标题',
        value: '',
    ...
    async (metadata, instance, item) => {
      ...
      await instance.value.show({
        ...
        request: (params) =>
          item
            ? doPopularMedicineUpdate({ id: item.id, ...params })
            : doPopularMedicineCreate(params),
      })
  ...
  // 删除
  const handleDelete = async (item: PopularMedicine) => {
    await ElMessageBox.confirm(`确认删除该数据？`)
    const { id } = item
    await doPopularMedicineDelete({ id })
    proTableInstance.value.reload()
    ElMessage.success('操作成功')
  }
</script>
