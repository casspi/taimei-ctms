<template>
  <ProTable :request="handleRequest" :query-metadata="queryMetadata" ref="proTableInstance">
    ...
    <ProTableActionsColumn :pds="[PD.social.advocateReport.check]" width="120">
      <template #default="scope">
        <ProTableActionButton
          :pd="PD.social.advocateReport.check"
          link
          type="warning"
...
<script setup lang="ts">
  ...
  import {
    DictionaryType,
    doAdvocateReportCheck,
    reqAdvocateReportList,
  ...
  const [queryMetadata, proTableInstance, handleRequest] = useProTable(
    ...
    async ([currentPage, pageSize], query) => {
      const { dataList, total } = await reqAdvocateReportList({ currentPage, pageSize, ...query })
  ...
  const [proDialogFormInstance, handleExamine] = useProDialogForm<ReportItem>(
    ...
    async (metadata, instance, item) => {
      ...
      await instance.value.show({
        title: '审核',
        metadata,
        request: (params) => doAdvocateReportCheck({ id: item!.id, ...params }),
      })
