<template>
  <ProTable :request="handleRequest" :query-metadata="queryMetadata" ref="proTableInstance">
    <ElTableColumn type="expand">
      <template #default="scope">
        <ElRow :gutter="20">
          <ElCol :span="12">
            <span>内容</span>
            <span>{{ scope.row.advocateContent }}</span>
          </ElCol>
          <ElCol :span="12">
            <span>图片</span>
    ...
    <ProTableActionsColumn :pds="[PD.social.advocate.check, PD.social.advocate.delete]" width="120">
      <template #default="scope">
        <ProTableActionButton
          :pd="PD.social.advocate.check"
          link
          type="warning"
...
<script setup lang="ts">
  ...
  const [queryMetadata, proTableInstance, handleRequest] = useProTable(
    ...
    async ([currentPage, pageSize], query) => {
      const { dataList, total } = await reqAdvocateList({ currentPage, pageSize, ...query })
  ...
  // 审核
  const [proDialogFormInstance, handleExamine] = useProDialogForm<Advocate>(
    () => ({
      createByName: {
        label: '发布人',
        value: '',
    ...
    async (metadata, instance, item) => {
      ...
      await instance.value.show({
        title: '审核',
        metadata,
        request: (params) => doAdvocateCheck({ id: item!.id, ...params }),
      })
  ...
  // 删除
  const handleDelete = async (item: Advocate) => {
    await ElMessageBox.confirm(`确认删除该数据？`)
    const { id } = item
    await doAdvocateDelete({ id })
    proTableInstance.value.reload()
    ElMessage.success('操作成功')
  }
</script>
