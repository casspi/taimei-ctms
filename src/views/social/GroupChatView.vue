<template>
  <ProTable :request="handleRequest" :query-metadata="queryMetadata" ref="proTableInstance">
    ...
    <ElTableColumn prop="publishDt" label="创建时间" />
...
<script setup lang="ts">
  ...
  const [queryMetadata, proTableInstance, handleRequest] = useProTable(
    ...
    async ([currentPage, pageSize], query) => {
      const { dataList, total } = await reqGroupChatList({ currentPage, pageSize, ...query })
  ...
  // 审核
  const [proDialogFormInstance, handleExamine] = useProDialogForm<GroupChat>(
    () => ({
      status: {
        label: '状态',
        value: '',
    ...
    async (metadata, instance, item) => {
      ...
      await instance.value.show({
        title: '审核',
        metadata,
        request: (params) => doGroupChatCheck({ id: item!.id, ...params }),
      })
  ...
  // 删除
  const handleDelete = async (item: GroupChat) => {
    await ElMessageBox.confirm(`确认删除该数据？`)
    const { id } = item
    await doGroupChatDelete({ id })
    proTableInstance.value.reload()
    ElMessage.success('操作成功')
  }
</script>
