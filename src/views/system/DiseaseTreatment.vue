<template>
  <ProTable :request="handleRequest" ref="proTableInstance">
    ...
    <ElTableColumn type="expand">
      <template #default="scope">
        <ElRow :gutter="20">
          <ElCol :span="12">
            <span>处方照</span>
            <template v-if="scope.row.prescriptionImagePathList?.length">
              <ElImage
                v-for="(item, index) in scope.row.prescriptionImagePathList"
                :key="index"
                class="c-img-preview-inline"
                :src="item"
                :initial-index="index"
                :preview-src-list="scope.row.prescriptionImagePathList"
                fit="cover"
                preview-teleported
              />
            </template>
    ...
    <ElTableColumn label="治疗信息" min-width="220">
      <template #default="scope">
        <ul>
          <li>治疗项目：{{ scope.row.projectName || '-' }}</li>
          <li>治疗日期：{{ scope.row.treatmentDt || '-' }}</li>
...
<script setup lang="ts">
  ...
  import { doDiseaseTreatmentHistoryDownload, reqDiseaseTreatmentHistory } from '@/api'
  ...
  const [, proTableInstance, handleRequest] = useProTable(
    ...
    async ([currentPage, pageSize], query) => {
      ...
      const { dataList, total } = await reqDiseaseTreatmentHistory({
        currentPage,
        pageSize,
        ...query,
      })
